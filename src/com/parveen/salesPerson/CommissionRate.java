package com.parveen.salesPerson;

import com.parveen.util.CurrencyConvertor;
...
public class CommissionRate extends salesPerson {
    ...
    public void display() {
        ...
        System.out.println("Commission: "+ CurrencyConvertor.CurrencyFormat(calculateTotalPay()+this.advance));
        System.out.println("Final Commission After deducting advance pay: "+CurrencyConvertor.CurrencyFormat(calculateTotalPay()));
        if (getPay()<0){
            setPay(-(this.pay));
            System.out.println("Amount that you have to reimburse "+CurrencyConvertor.CurrencyFormat(getPay())+" to the company ");
        }
    }
}
